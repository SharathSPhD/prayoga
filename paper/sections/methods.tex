This section describes the models and substrate, the refusal-direction extraction
and interventions, the outcome measures, and the statistical controls that gate every
claim. Methods specific to a single result are stated with that result.
...
$m$, compared against a random-direction control, and symmetry-breaking is measured
as the drop in $m$ between a plain harmful request and its injection-framed
counterpart. Building on the same machinery, each of the six \d{s}a\d{t}karma acts is
operationalized as a distinct activation intervention with
