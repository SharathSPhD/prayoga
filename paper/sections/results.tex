...
behavioural flip?---is resolved \emph{negatively} once the behavioural label is valid.
A first pass with a substring refusal metric was inconclusive, and inspection showed
\emph{why}: that metric scores \texttt{indirect\_injection} deflections (``this document
does not provide\ldots'') as compliance. Replacing it with a content-faithful LLM-judge
(Claude classifying whether the output actually provides the harmful content) cuts the
flip rate by roughly $18\times$: genuine behavioural capture under prompt-level injection
is only $1$--$2.4\%$ across the three models, with \texttt{indirect\_injection} and
\texttt{many\_shot} at $0\%$ despite collapsing the internals the most. Thus internal
monitor-collapse is \emph{not sufficient} for behavioural capture under context injection:
the last-token order parameter and precision margin are faithful internal signals but not
the policy controller (genuine capture in these models requires weight-level ablation,
not context injection). This bounds the unification claim to the representational level
and, as a methodological corollary, shows the substring refusal metric is an invalid
behavioural DV---retroactively bounding behavioural readouts elsewhere in the program. A
geometric sub-study (removing the diff-in-means axis collapses linear separability in all
three families) refines but does not adjudicate the F6/F8 dimensionality account, and we
report it as descriptive only.

\subsection{Source-of-truth: a genuine truthfulness direction behind the falsified state probe}
Falsification is a starting point, not an endpoint. The avasth\=atraya j
\=agrat/svapna ``generation regime'' was falsified (no mid-network gain over a layer-0
surface baseline). Re-testing the underlying question with two \emph{independent} sets of
balanced true/false declaratives, we find a genuine linear truth-evaluation feature:
in Gemma-2-2b a mid layer (13) classifies statement truth and \emph{transfers across a
different topic set} at $0.96$ (within-set CV $1.00$), while the surface layer-0 baseline
is at chance ($0.50$); the label-shuffle null gives $p=0.002$. This recovers the
Azaria--Mitchell / Marks--Tegmark truthfulness direction under the program's stronger
cross-dataset and surface gates. Crucially it is a feature of \emph{input} factuality, not
a generation regime, so the avasth\=atraya state reading remains falsified; the positive
result is reported at the mechanism tier with no machine-state upgrade.

Applying the same discipline to the cross-family \emph{addition asymmetry} (refusal
addition succeeds in Gemma but was reported to fail in Qwen even at $64\times$) overturns
its interpretation. Two geometric explanations fail---removing the diff-in-means axis
collapses separability in all families, and the alignment $\cos(\hat d, w)$ between that
axis and the probe normal is high everywhere and \emph{highest} in Qwen ($0.91$). A
coefficient sweep gives the real answer: adding the single direction at coefficient $16$
induces clean, inspection-verified over-refusal in Qwen ($+1.0$), while coefficient $64$
pushes activations off-distribution into incoherent text (scored as non-refusal). The
``Qwen addition fails'' result was thus a dose-calibration artifact, not model-specific
insufficiency; single-direction addition is sufficient in both families at the appropriate
coefficient, strengthening the representational-level unification while F20 bounds it
behaviourally.

Finally, a methodological caution that emerged from validating these behavioural
readouts: the two automatic dependent variables are biased in \emph{opposite} directions.
The substring refusal metric \emph{over}-counts compliance (it scores deflections as
success, inflating the injection flip rate $\sim$18$\times$); a content-faithful judge
built from a safety-trained model (Claude) \emph{under}-counts it (it refuses to affirm
genuinely harmful outputs---on ablated Gemma-2-2b, inspection confirms genuine harmful
generations and a substring ASR of $0.92$, yet the judge scores only $0.05$). Neither is
an unbiased behavioural DV: the substring metric is an upper bound on capture and the
safety-trained judge a lower bound, so behavioural claims here are bracketed by both and
spot-checked by inspection. (This validates the ablation finding---ablation produces
genuine harmful compliance---and makes the injection capture rate a lower bound.)
